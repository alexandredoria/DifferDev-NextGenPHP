...
class Cookie implements HeaderStringInterface
{
    ...
    public function setExpires(DateTimeImmutable $startDate = new DateTimeImmutable('now'), Expires $expiresComponent): void
    {
        $dateInterval = $startDate->add(
    ...
    {
        if (false === filter_var("http://$domain", FILTER_VALIDATE_URL)){
            throw new InvalidArgumentException;
        }
    ...
    public function setMaxAge(int $seconds): void
    {
        ...
        $this->parameters[] = "Max-Age=$seconds";
    }

    public function setPartitioned(): void
    {
        $this->parameters[] = "Partitioned";
    }

    public function setPath(string $path): void
    {
        if (mb_detect_encoding($path) !== 'ASCII') {
            throw new InvalidArgumentException;
        }
    ...
    {
        $this->parameters[] = "Secure";
    }

    public function setSameSite(string $value): void
    {
        if (false === in_array($value, ['Strict', 'Lax', 'None'])){
            throw new InvalidArgumentException;
        }

        $this->parameters[] = "SameSite=$value";
    }
